...
CREATE TABLE `G_ORDER` (
  ...
  `Address_id` int(11) DEFAULT NULL REFERENCES `ADDRESS` (`Address_id`),
  `user_id` varchar(20) NOT NULL  REFERENCES `USER` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `PRODUCT` (
  ...
  `Manufacturer_id` int(11) NOT NULL REFERENCES `MANUFACTURER` (`Manufacturer_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `PRODUCT_ORDER` (
  `Product_id` int(11) DEFAULT NULL REFERENCES `PRODUCT` (`Product_id`),
  `Order_id` int(11) DEFAULT NULL REFERENCES `G_ORDER` (`Order_id`),
